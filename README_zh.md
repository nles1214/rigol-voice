...
## v0.3.1新增功能特性
- **单次出发循环模式** —单次触发模式下，远程自动检测是否被触发，检测到触发波形后，自动截图保存到本地项目文件夹，冷却30s后自动恢复单次触发模式，进入下一个循环。这样示波器就可以整夜在实验室检测测试波形并自动划波形，存波形了。
<img width="1280" height="800" alt="scope_20260602_204757" src="https://github.com/user-attachments/assets/9748215d-ed30-4c43-870b-062a3cc78875" />
...
- **自动截图模式** — 单次触发循环：检测波形 → 自动截图保存 → 30 秒冷却 → 重新进入单次触发，全程无需手动操作
- **语音识别**
...
| 标签 | 内容 |
|------|------|
| **v0.3.1** | 自动截图模式：单次触发循环 + 自动截图 + 30 秒冷却倒计时 |
